...
def create_graph(username: str, password: str, ip: str) -> None:
    ...
    if not db.has_graph(GRAPH):
        norb_graph = db.create_graph(GRAPH)
    else:
        norb_graph = db.graph(GRAPH)

    # Create vertex collections
    for vertex in NODE_KEYS:
        if not norb_graph.has_vertex_collection(vertex):
            _ = norb_graph.vertex_collection(vertex)
    ...
    for edge_key in edge_keys:
        ...
        if not norb_graph.has_edge_definition(edge_collection_key):
            _ = norb_graph.create_edge_definition(
...
def main(norb_file_path: str, username: str, password: str, ip: str) -> None:
    ...
    # Load data
    nx_norb_graph = load_graph_network(norb_file_path)
    logging.info("Loaded nx")
    # Insert nodes
    nodes_nx = nx_norb_graph.nodes(data=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ip", type=str, required=True,
                        help="DB IP address")
    parser.add_argument("--arango_import", action='store_true', help="Use arangoimport with created json files from NORB. Requires `arangoimport`.")
    ...
    parser.add_argument("--create_db", action='store_true', help="Create NORB db")
