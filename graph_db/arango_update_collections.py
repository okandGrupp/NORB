...
from graph_db.norb_arango import (
    DB,
)
...
from NORB.build_NORB import build_graph, parse_args as bb_parse_args, NORB_PATH
from graph_db.norb_arango import main as ba_main, arango_import
...
NORB_SAVE_PATH = 'update_norb_data'
...
def main(username: str, password: str, ip: str) -> None:
    ...
    os.makedirs(NORB_SAVE_PATH, exist_ok=True)
    NORB_original_id_to_norb_id_path = os.path.join(NORB_SAVE_PATH, NORB_PATH)
    os.makedirs(NORB_original_id_to_norb_id_path, exist_ok=True)
    bb_args = {'input_data_folder': OUTPUT_FOLDER,
            'save_path': NORB_SAVE_PATH,
            }
    print(bb_args)
    build_graph(**bb_args)
    logging.info("Built NORB")

    # NORB to arango format
    norb_json_path = os.path.join(NORB_SAVE_PATH, "NORB.json")
    ba_main(norb_json_path, username, password, ip)
    logging.info(f"Created arango format from {norb_json_path}")
    # Import new NORB files
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Create json files to import into ArangoDb from NORB json')
