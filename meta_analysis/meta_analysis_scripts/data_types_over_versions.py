...
def vendor_product_versions(NORB_path, vendor, product):
    """
    Return list of versions for vendor product in order from oldest to most recent
    """
    if NORB_path.lower().endswith('.json'):
        with open(NORB_path) as f:
            graph = json.load(f)
    elif NORB_path.lower().endswith('.gz'):
        with gzip.open(NORB_path, "rt", encoding="utf-8") as f:
            graph = json.load(f)
...
def data_types_over_versions(NORB_folder_path, vendor, product, starting_point_file, search_result_file, save_path=None):
    NORB_path = os.path.join(NORB_folder_path, "NORB.json")
    plt.rc('font', size=14)
    plt.rcParams["font.weight"] = "bold"
    plt.rcParams["axes.labelweight"] = "bold"
    # Save file of starting points for vendor product
    _, id_to_version = version_to_cpe_ids(NORB_folder_path, vendor, product, starting_point_file, CPE_ID_NORB_ID_PATH)
    # Run path search on starting points, save file of search results for vendor product
    cmd = f"python -m path_search.path_search_NORB --NORB_path {NORB_path} --starting_point {starting_point_file} --starting_point_type 'cpe' --results_file {search_result_file}"
    sys = os.system(cmd)
...
def parse_args(args: List[str]) -> Dict[str, Any]:
    ...
    parser.add_argument('--NORB_folder_path', type=str, required=True,
                        help='Folder path to NORB graph and files')
...
def main(**args: Dict[str, Any]) -> None:
    NORB_folder_path, vendor, product, starting_point_file, search_result_file, save_path = args.values()
    data_types_over_versions(NORB_folder_path, vendor, product, starting_point_file, search_result_file, save_path=save_path)
