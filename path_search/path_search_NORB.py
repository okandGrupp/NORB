...
def make_graph_edges(data_key, data_type, norb_dict, edge_dict, graph, rows_list):
    ...
    if data_key in norb_dict:
        ...
        name = f"{data_key}: {data_type}_{norb_id}"
...
def get_chain(data, graph, data_type, length, NORB_folder_path):
    rows_list = []
    if data_type == "tactic":
...
def main_attack(NORB_folder_path, args_file, results_name, arg_type, length):
    graph_path = os.path.join(NORB_folder_path, "NORB.json")
    graph = load_graph_network(graph_path)
    data = get_data(args_file)
    network_data_frame = get_chain(data, graph, arg_type, length, NORB_folder_path)
...
def parse_args(args: List[str]) -> Dict[str, Any]:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to NORB e.g. data/NORB.json",
    )
    parser.add_argument(
        "--length",
        action="store_true",
...
def main(**args: Dict[str, Any]) -> None:
    NORB_folder_path, length, starting_point, results_file, starting_point_type = args.values()
    main_attack(NORB_folder_path, starting_point, results_file, starting_point_type, length)
